...
如果需要提交演示视频，建议控制在 3 到 5 分钟，不需要配音，按下面顺序录制：

1. 展示 `.env` 中已填写 `LLM_PROVIDER` 和 API Key，占位或打码均可。
2. 在终端执行 `docker compose up --build`，展示服务启动成功。
3. 浏览器打开 `http://localhost:5173`。
4. 添加 `demo` 数据库连接。
5. 演示一个成功查询，例如“按订单状态统计订单数量和总金额”。
6. 演示一个安全拦截，例如“删除所有订单”。
7. 切换到慢查询分析，点击 `Use sample` 和 `Analyze`。
8. 展示最终结果页面和查询输出。
